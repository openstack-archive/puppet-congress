...
describe 'congress::logging' do
  ...
  let :log_params do
    {
     ...
     :log_dir => '/var/log',
     :log_file => '/var/tmp/congress_random.log',
     :debug => true,
    }
  end
  ...
  shared_examples 'basic default logging settings' do
    it 'configures congress logging settings with default values' do
      is_expected.to contain_oslo__log('congress_config').with(
        :use_syslog          => '<SERVICE DEFAULT>',
        :use_json            => '<SERVICE DEFAULT>',
        :use_stderr          => '<SERVICE DEFAULT>',
        :syslog_log_facility => '<SERVICE DEFAULT>',
        :log_dir             => '/var/log/congress',
        :log_file            => '/var/log/congress/congress.log',
  ...
  shared_examples 'basic non-default logging settings' do
    it 'configures congress logging settings with non-default values' do
      is_expected.to contain_oslo__log('congress_config').with(
        :use_syslog          => true,
        :use_json            => true,
        :use_stderr          => false,
  ...
  shared_examples_for 'logging params unset' do
   [ :logging_context_format_string, :logging_default_format_string,
     ...
     :log_date_format, ].each { |param|
        it { is_expected.to contain_oslo__log('congress_config').with("#{param}" => '<SERVICE DEFAULT>') }
      }
  end
